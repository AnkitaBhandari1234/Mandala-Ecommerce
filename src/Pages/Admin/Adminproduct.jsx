...
const Adminproduct = () => {
  ...
  useEffect(() => {
    ...
    // Check every second if a product was added recently
    const interval = setInterval(() => {
      // Get the last update time from localStorage
      const updateTime = localStorage.getItem("productUpdate");

      // If there is an update time and it was set within the last 3 seconds
      if (updateTime && Number(updateTime) > Date.now() - 3000) {
        fetchProducts(); // Reload the products
        localStorage.removeItem("productUpdate"); // Clear the update signal
      }
    }, 1000); // Run this check every 1000 milliseconds (1 second)
